    ...
    public class MazeGen // Declara la clase pública MazeGen
    {
        ...
        private void GenerateMaze() // Método para generar el laberinto
        {
            ...
            foreach (var (dy, dx) in directions) // Itera sobre las direcciones posibles
            {
                ...
                if (PosicionValida(ny, nx)) // Verifica si las coordenadas están dentro del laberinto
                {
            ...
            while (walls.Count > 0) // Mientras haya paredes en la lista
            {
                ...
                if (Maze[y, x] == "🌳") // Si la celda es una pared
                {
                    ...
                    foreach (var (dy, dx) in directions) // Itera sobre las direcciones posibles
                    {
                        ...
                        if (PosicionValida(ny, nx) && Maze[ny, nx] == "⬜") // Verifica si las coordenadas están dentro del laberinto y son camino
                        {
                    ...
                    if (neighbors.Count == 1) // Si hay exactamente un vecino
                    {
                        ...
                        foreach (var (dy, dx) in directions) // Itera sobre las direcciones posibles
                        {
                            ...
                            if (PosicionValida(nwy, nwx)) // Verifica si las coordenadas están dentro del laberinto
                            {
       ...
       public static bool PosicionValida(int row, int col) // Método para verificar si una celda está dentro del laberinto
        {
            ...
            for (int j=0; j<Dimension; j++)
            {
                if(Maze[i,j]=="⬜")
                {
                    ...
                    int Randdir=random.Next(0, 4);
                        if (PosicionValida(i+directions[Randdir].Item1, j+directions[Randdir].Item2))
                        {
        ...
        private bool CheckConnected(int i, int j, bool[,] b){
            foreach(var  (dy, dx) in directions)
                    {
                      if(PosicionValida(i+dx, j+dy) &&  Maze[i+dx, j+dy]=="⬜" && b[i+dx, j+dy]==false)
                      {
                        return true;
                      }
                    }
                ...
                for(int j=0; j<Dimension; j++)
                {
                    ...
                    if (posibilidad==1){
                        ...
                        if(SquareEmojis.Contains(Maze[i, j]))
                        {
                            ...
                            {
                              if(PosicionValida(i+dir.Item1, j+dir.Item2) && Maze[i+dir.Item1, j+dir.Item2]=="⬜")
                              {
